...
PString Conference::SaveTemplate(PString tplName)
{
  ...
  { t << "  MIXER " << vmr->id << "\n"
    ...
    VMPCfgSplitOptions & o = MyProcess::vmcfg.vmconf[n].splitcfg;
...
//            if(MyProcess::Current().GetEndpoint().MakeCall(memberAddress, token, userData) != NULL)
...
//              MyProcess::Current().HttpWriteEventRoom(msg,number);
          ...
          if(!offline) // online: just tune him up
          {
            ...
            //MyProcess::Current().GetEndpoint().SetMemberVideoMixer(*this, r->second, v[4].AsInteger());
          }
...
//        MyProcess::Current().GetEndpoint().SetMemberVideoMixer(*this, member, v[4].AsInteger());
